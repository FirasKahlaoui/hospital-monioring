...
The healthcare sector is undergoing a significant transformation driven by Internet of Things (IoT) technologies.
...
\begin{itemize}
	\item \textbf{Embedded acquisition node (ESP32):} real-time sensor acquisition and authenticated cloud transmission.\cite{esp32_datasheet,dht22_datasheet,max30102_datasheet}
	...
	\item \textbf{AI insights layer:} statistical anomaly detection and short-horizon
	      trend forecasting executed on the server and exposed through a typed API.
\end{itemize}
...
\begin{enumerate}
	\item Availability: the system shall operate continuously with network recovery mechanisms.
...
\begin{description}
	\item[Embedded acquisition node (ESP32).]
	      ...
	      including resolution of the MAX30102 pull-up resistor incompatibility.\cite{esp32_datasheet,dht22_datasheet,max30102_datasheet}

	\item[Backend server.]
	      Design and deployment of a Node.js/tRPC service layer with Firebase Admin SDK
	      integration, Role-Based Access Control (RBAC), Zod schema validation, event
	      and alert log generation for presence monitoring, a statistical AI insights
	      endpoint for vitals, and an SMTP email procedure triggered by dashboard alerts.
\cite{trpc,firebase_auth,firebase_firestore,firebase_rtdb}
	...
	\item[Web dashboard.]
		      ...
		      panel, and an on-device face-api.js pipeline running in the browser for
		      authorized personnel tracking using an external USB camera.\cite{react,recharts,faceapi,firebase_rtdb}

	\item[AI insights layer.]
		      Server-side implementation of a statistical anomaly detection engine (Z-score
		      over a short sliding window of recent points, with a minimum of 5 samples)
		      and a 5-step vital sign forecasting model (Ordinary Least Squares linear
		      regression), exposed as tRPC procedures and integrated into the dashboard's
		      clinical status display.\cite{trpc}
\end{description}
