...
browser-native technologies — WebSockets and the Recharts rendering
engine — are sufficient for a responsive clinical monitoring interface with concurrent
AI inference. The AI layer demonstrates that simple statistical methods (Z-score
anomaly detection and OLS linear regression) can deliver clinically meaningful
early warnings with low computational cost; precision must be validated with
clinical datasets.

All conclusions in this report are grounded in the prototype implementation and the validation plan defined in Chapter~6; metrics not yet measured are explicitly marked as pending.
...
limits. Data transmission to Firebase RTDB operates on a 1\,s push interval, with
...
The non-functional objectives are addressed by design: the system recovers
autonomously from Wi-Fi loss via periodic reconnect attempts (availability); the
...
(performance). Formal measurements remain pending.
