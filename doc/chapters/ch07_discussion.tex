...
DHT22\cite{dht22_datasheet} ($\pm$0.5\,°C, $\pm$2--5\,\% RH) and within the expected operational range of
the MAX30102\cite{max30102_datasheet} for resting-state adult subjects.
...
The backend procedures are structurally consistent with the tRPC\cite{trpc} schema and are
successfully validated via the server-side caller. Procedures for person management,
event logging, and AI inference operate as expected, with strict TypeScript type
safety across the boundary. The AI
...
\begin{description}
	...
	\item \textbf{Credential Protection.}
	      Firebase\cite{firebase_firestore,firebase_rtdb} credentials (API key, email, password) are stored in NVS without
	      encryption in the current prototype.
	      The ESP32\cite{esp32_datasheet} NVS partition supports native encryption via an NVS key partition;
	      this was not implemented for the MVP but is a high-priority security
	      improvement.
...
\begin{description}
	...
	\item[Single-region deployment.] The backend and Firebase\cite{firebase_firestore,firebase_rtdb} services are currently deployed in a single geographic region, increasing latency for cross-regional users and lacking multi-region failover.
\end{description}
...
% ============================================================
\section{Critical Analysis of System Architecture}
% ============================================================
\subsection{Real-Time Reliability and Cloud Dependency Risks}
The system's reliance on Firebase Realtime Database for telemetry introduces a strict cloud dependency. While average latencies remain under 300\,ms, a severed internet connection breaks the monitoring loop. Unlike traditional local-network hospital systems (e.g., HL7 over LAN), this cloud-first approach trades local fault tolerance for global accessibility. A local fallback server or an edge gateway is highly recommended for life-critical deployment scenarios.

\subsection{Browser Inference Limitations}
Offloading face recognition to the client browser (\texttt{face-api.js}) successfully reduces backend compute costs and scales infinitely with the number of client terminals. However, it tightly couples monitoring performance to the clinician's hardware. On lower-end hospital tablets, executing WebGL models concurrently with 60\,FPS SVG charting causes measurable UI stutter. Migrating the inference pipeline to a Web Worker or WebAssembly (Wasm) thread would prevent main-thread blocking.
